% Diagram for the Anduril 2 user interface for flashlights
% Copyright (C) 2024-2026 Guido dirtydancing
% SPDX-License-Identifier: GPL-3.0-or-later

% Preamble - Table of Contents -----------------------------------------
% ----------------------------------------------------------------------

% Preamble - 01 Layout -------------------------------------------------
% Preamble - 02 xcolor -------------------------------------------------
% Preamble - 03 tikz ---------------------------------------------------
% Preamble - 04 tabularray ---------------------------------------------
% Preamble - 05 tcolorbox ----------------------------------------------
% Preamble - 06 Misc ---------------------------------------------------

% Preamble - 01 Layout -------------------------------------------------
% ----------------------------------------------------------------------
\documentclass[8pt]{extarticle} % for font size 8pt
% \documentclass[10pt]{article} % default font size: 10pt

\usepackage[
    a4paper, % for paper size
    landscape, % for page orientation
    margin=0.6cm % for page margins
]
{geometry} % for document dimensions

\usepackage{nopageno} % for an empty page style on all pages

\usepackage{fontspec} % for loading OpenType fonts
\setmainfont{Source Sans 3} % successor to Source Sans Pro

\usepackage{setspace} % for setting the line spacing
\setstretch{0.9} % default: single spacing

% for vertical spacing in text
\newcommand{\spacingv}{\\[2pt]}

% for one line of text
\newcommand{\oneline}[1]{\bfseries\normalsize{#1}}

% for the first line of more than one line of text
\newcommand{\firstline}[1]{\oneline{#1}\spacingv}

% for horizontal spacing between nodes
\newcommand{\nodespaceh}{0.6cm}

% for vertical spacing between nodes
\newcommand{\nodespacev}{0.25cm}

% for border spacing
\newcommand{\borderspace}{0.3cm}

% Preamble - 02 xcolor -------------------------------------------------
% ----------------------------------------------------------------------

% \PassOptionsToPackage{gray}{xcolor} % for grayscale output

\usepackage{xcolor} % for access to colors and for \pagecolor
% xcolor package is also loaded by tikz package

% any = for Simple UI and Advanced UI
\colorlet{any-draw}{black}
\colorlet{anyconfig-draw}{any-draw}
\colorlet{anyconfig-fill}{yellow!30}
\colorlet{any-text}{any-draw}
\colorlet{anybutton-draw}{anyconfig-draw}
\colorlet{anybutton-fill}{white}
\colorlet{anybutton-text}{any-text}

% advanced = for Advanced UI only
\colorlet{advanced-draw}{any-draw}
\colorlet{advancedconfig-draw}{gray!75}
\colorlet{advancedconfig-fill}{gray!10}
\colorlet{advanced-text}{advanced-draw}
\colorlet{advancedbutton-fill}{anybutton-fill}
\colorlet{advancedbutton-text}{advanced-text}
\colorlet{advancedUI-draw}{advanced-draw}

% for Advanced UI - config menus
\colorlet{advancedconfigmenu-draw}{advancedconfig-draw}
\colorlet{advancedconfigmenu-fill}{advancedconfig-fill}
\colorlet{advancedconfigmenu-text}{advanced-text}

% for Advanced UI - Simple UI config menu
\colorlet{simpleUIconfigmenu-draw}{advancedconfigmenu-draw}

% simple = for Simple UI only
\colorlet{simple-draw}{any-draw}
\colorlet{simplebutton-double}{anybutton-fill}
\colorlet{simpleUI-draw}{any-draw}
\colorlet{simple-text}{any-text}

% for UI switch lines
\colorlet{advancedUIswitchline-draw}{orange}
\colorlet{simpleUIswitchline-draw}{advancedUIswitchline-draw}

% for Extended Simple UI
\colorlet{extendedsimpleUI-draw}{any-draw}
\colorlet{extendedsimpleUI-fill}{anybutton-fill}

% for On, Ramp, Off, and Lockout
\colorlet{onoff-draw}{any-draw}
\colorlet{onoffbutton-fill}{anybutton-fill}
\colorlet{onoff-text}{any-text}
\colorlet{onofframplabel-draw}{onoff-draw}
\colorlet{onofframplabel-fill}{onoffbutton-fill}
\colorlet{onofframplabel-text}{onoff-text}
\colorlet{onofftogglerampstyle-draw}{onoff-draw}
\colorlet{onofftogglerampstyle-fill}{onoffbutton-fill}
\colorlet{onofftogglerampstyle-text}{onoff-text}

% any... mode = for Batt Check from OFF
\colorlet{anyconfigmode-draw}{anyconfig-draw}

% advanced... mode = for strobe modes and blinky modes
\colorlet{advancedconfigmode-draw}{advancedconfig-draw}
\colorlet{advancedbordermode-draw}{advancedconfigmode-draw}

% for basic use
\colorlet{basicuse-draw}{blue!80}
\colorlet{anybuttonbasicuse-draw}{basicuse-draw}
\colorlet{anybuttonbasicuse-fill}{anybutton-fill}
\colorlet{basicuse-text}{any-text}
\colorlet{buttonpressesbasicuse-draw}{basicuse-draw}
\colorlet{fillarrowbasicuse}{anybutton-fill}

% for ramp image
\colorlet{rampsmooth-topbottom}{black!40}
\colorlet{rampstepped-topbottom}{rampsmooth-topbottom}
\colorlet{rampcombinedinside-draw}{anyconfig-fill}

% for ramp text Memory, Ceiling, and Floor
\colorlet{ramptextconfig-fill}{anyconfig-fill}

% for Factory Reset
\colorlet{factoryreset-draw}{advancedUIswitchline-draw}
\colorlet{factoryreset-fill}{orange!20}

% for line from Factory Reset to Simple UI
\colorlet{factoryresetsimpleUI-draw}{factoryreset-draw}

% for general information
\colorlet{generalinfo-draw}{advancedbordermode-draw}
\colorlet{generalinfo-fill}{anybutton-fill}
\colorlet{andurilinfo-draw}{generalinfo-draw}
\colorlet{andurilinfo-fill}{generalinfo-fill}
\colorlet{andurilinfo-text}{any-text}
\colorlet{voltagecolorinfo-fill}{generalinfo-fill}
\colorlet{buttonpressesinfo-fill}{generalinfo-fill}
\colorlet{defaultsvaryinfo-fill}{generalinfo-fill}
\colorlet{matrixlegendinfo-fill}{generalinfo-fill}
\colorlet{usermanualinfo-draw}{cyan!80}
\colorlet{usermanualinfo-fill}{generalinfo-fill}

% for voltage colors
\colorlet{voltagebox-frame}{anybutton-fill}
\colorlet{voltagebox-back}{voltagebox-frame}
\colorlet{voltagebox-mixfill}{voltagebox-frame}
\colorlet{voltageboxcolor-red}{red!20!voltagebox-mixfill}
\colorlet{voltageboxcolor-yellow}{yellow!20!voltagebox-mixfill}
\colorlet{voltageboxcolor-green}{green!20!voltagebox-mixfill}
\colorlet{voltageboxcolor-cyan}{cyan!20!voltagebox-mixfill}
\colorlet{voltageboxcolor-blue}{blue!20!voltagebox-mixfill}
\colorlet{voltageboxcolor-purple}{purple!20!voltagebox-mixfill}
\colorlet{voltagebox-text}{any-text}
\colorlet{voltageboxadv-text}{advanced-text}

% for tint ramp colors
\colorlet{tintrampbox-frame}{voltagebox-frame}
\colorlet{tintrampbox-mixfill}{voltagebox-mixfill}
\colorlet{tintrampboxcolor-left}{orange!20!tintrampbox-mixfill}
\colorlet{tintrampboxcolor-right}{blue!12!tintrampbox-mixfill}
\colorlet{tintrampbox-text}{voltagebox-text}
\colorlet{tintrampboxadv-text}{voltageboxadv-text}

% for smooth POVD
\colorlet{smoothPOVDbox-frame}{voltagebox-frame}
\colorlet{smoothPOVDbox-text}{voltagebox-text}
\colorlet{smoothPOVDboxadv-text}{voltageboxadv-text}

% for HSV channel mode
\colorlet{HSVchannelmodebox-frame}{voltagebox-frame}
\colorlet{HSVchannelmodeboxadv-text}{voltageboxadv-text}

% for infotext
\colorlet{infoany-text}{any-text}
\colorlet{infoadvanced-text}{advanced-text}
\colorlet{infoadvancedconfigmenu-text}{advancedconfigmenu-text}
\colorlet{infosimple-text}{simple-text}

% for on lights with dimmable RGB aux LEDs
\colorlet{dimmableRGB-color1}{red!77}
\colorlet{dimmableRGB-color2}{red!55}
\colorlet{dimmableRGB-color3}{red!33}

% for inherited by Simple UI
\colorlet{inherited-text}{advanced-text}
\colorlet{inherited-color}{inherited-text}

% for setting the page background color
% \pagecolor{anybutton-fill} % default: transparent background

% Preamble - 03 tikz ---------------------------------------------------
% ----------------------------------------------------------------------
\usepackage{tikz} % for creating graphics
% tikz package is also loaded by tcolorbox package

\usetikzlibrary{arrows.meta} % for Stealth arrow tip

\usetikzlibrary{bending} % for flexing and bending arrows
% with bending library, default: flex
% without bending library, default: quick

\usetikzlibrary{positioning} % for relative positioning

\usetikzlibrary{shapes.misc} % for rounded rectangle

\usetikzlibrary{fit} % for fitting a node

\usetikzlibrary{backgrounds} % for background frames
% backgrounds library is also loaded by tikz-layers package

\usetikzlibrary{decorations.text} % for text along path

\usetikzlibrary{matrix} % for matrix of nodes

\usetikzlibrary{scopes} % for a shorthand for {scope} environments
% for layers: { [on background layer] ... }

\usetikzlibrary{calc} % for coordinate calculations

\usetikzlibrary{tikzmark} % for marking a point for further use

% for arrow tips
\tikzset{
    >={
        Stealth[
            round,
            scale length=1.25,
            scale width=1.25
        ]
    }
}

% any = for Simple UI and Advanced UI
\tikzset{
    anyline/.style={
        draw=any-draw,
        thin % thin = 0.4pt (default)
    },
    anyline ramp/.style={
        anyline,
        line cap=round
    },
    anyline momentary/.style={
        anyline,
        densely dashed,
        line cap=round
    },
    anyconfig/.style={
        draw=anyconfig-draw,
        thin,
        solid,
        fill=anyconfig-fill,
        text=any-text,
        inner sep=0.1cm,
        rounded corners=4.5pt, % default: 4pt
        align=left
    },
    anybutton/.style={
        draw=anybutton-draw,
        thin,
        solid,
        fill=anybutton-fill,
        text=anybutton-text,
        font=\normalsize,
        inner sep=0.075cm,
        rounded corners
    }
}

% advanced = for Advanced UI only
\tikzset{
    advancedline/.style={
        anyline,
        draw=advanced-draw
    },
    advancedline momentary/.style={
        advancedline,
        densely dashed,
        line cap=round
    },
    advancedconfig/.style={
        anyconfig,
        draw=advancedconfig-draw,
        fill=advancedconfig-fill,
        text=advanced-text
    },
    advancedbutton/.style={
        anybutton,
        draw=none,
        fill=advancedbutton-fill,
        text=advancedbutton-text
    },
    advancedUI/.style={
        advancedconfig,
        draw=advancedUI-draw,
        very thick, % very thick = 1.2pt
        font=\bfseries\LARGE
    }
}

% for Advanced UI - config menus
\tikzset{
    advancedconfigmenu/.style={
        advancedconfig,
        draw=advancedconfigmenu-draw,
        fill=advancedconfigmenu-fill,
        text=advancedconfigmenu-text
    }
}

% for Advanced UI - Simple UI config menu
\tikzset{
    simpleUIconfigmenu/.style={
        advancedconfigmenu,
        draw=simpleUIconfigmenu-draw
    }
}

% simple = for Simple UI only
\tikzset{
    simpleline/.style={
        anyline,
        draw=simple-draw
    },
    simpleline momentary/.style={
        simpleline,
        densely dashed,
        line cap=round
    },
    simplebutton/.style={
        anybutton,
        double=simplebutton-double % default: white (core color)
    },
    simpleUI/.style={
        anyconfig,
        draw=simpleUI-draw,
        very thick,
        font=\bfseries\LARGE
    }
}

% for UI switch lines
\tikzset{
    advancedUI switch line/.style={
        simpleline,
        draw=advancedUIswitchline-draw,
        very thick
    },
    simpleUI switch line/.style={
        advancedline,
        draw=simpleUIswitchline-draw,
        very thick
    }
}

% for Extended Simple UI
\tikzset{
    extendedsimpleUI/.style={
        anyconfig,
        draw=extendedsimpleUI-draw,
        fill=extendedsimpleUI-fill
    }
}

% for On, Ramp, Off, and Lockout
\tikzset{
    on-extra/.style={
        draw=onoff-draw,
        thick, % thick = 0.8pt
        fill=onoffbutton-fill,
        text=onoff-text,
        font=\bfseries\normalsize,
        inner xsep=0.375cm,
        inner ysep=0.065cm,
        rounded rectangle
    },
    on-extra ramp label/.style={
        on-extra,
        draw=onofframplabel-draw,
        fill=onofframplabel-fill,
        text=onofframplabel-text,
        inner xsep=0.35cm
    },
    on-extra toggle ramp style/.style={
        on-extra,
        draw=onofftogglerampstyle-draw,
        fill=onofftogglerampstyle-fill,
        text=onofftogglerampstyle-text
    },
    onoff/.style={
        on-extra,
        font=\bfseries\Huge,
        on grid
    },
    on-rampimage/.style={
        onoff,
        inner xsep=0.14cm
    }
}

% for Lockout to Off right arrows
\tikzset{
    lockoutoffright-arrow/.style={
        draw=none,
        fill=none,
        inner sep=0cm,
        minimum width=0.28cm,
        minimum height=0.05cm
    }
}

% any... mode = for Batt Check from OFF
\tikzset{
    anyconfig-mode/.style={
        anyconfig,
        draw=anyconfigmode-draw
    }
}

% advanced... mode = for strobe modes and blinky modes
\tikzset{
    advancedline-mode/.style={
        advancedline
    },
    advancedbutton-mode/.style={
        advancedbutton
    },
    advancedconfig-mode/.style={
        advancedconfig,
        draw=advancedconfigmode-draw
    },
    advancedborder-mode/.style={
        advancedconfig,
        draw=advancedbordermode-draw,
        fill=none
    }
}

% for basic use
\tikzset{
    anyline basic use/.style={
        anyline,
        draw=basicuse-draw,
        very thick
    },
    anybutton basic use/.style={
        anybutton,
        draw=anybuttonbasicuse-draw,
        line width=2pt,
        fill=anybuttonbasicuse-fill,
        text=basicuse-text,
        font=\bfseries
    },
    button presses basic use fit/.style={
        advancedborder-mode,
        draw=buttonpressesbasicuse-draw,
        thick,
        inner xsep=0.03cm,
        inner ysep=0.05cm,
        rounded corners=2.25pt
    }
}

% for ramp image
\tikzset{
    rampstepped/.style={
        rectangle,
        draw=none,
        inner sep=0cm,
        outer xsep=-0.02pt, % -0.02pt for a cleaner look
        outer ysep=-0.1pt, % -0.1pt for a cleaner look
        minimum width=0.66cm+0.3pt,
        minimum height=0.9cm+0.1pt+0.2pt % +0.2pt for a cleaner look
    },
    rampsmooth/.style={
        rampstepped,
        minimum width=0.66cm+0.3pt,
        minimum height=5.4cm+0.6pt+0.2pt % +0.2pt for a cleaner look
        % 5.4cm = 6 x 0.9cm
    },
    rampcombined/.style={
        rectangle,
        draw=any-draw,
        thin,
        fill=none,
        inner sep=0cm,
        outer sep=0cm,
        minimum width=1.32cm+7.2pt,
        minimum height=5.4cm+7.2pt,
        rounded corners=0.25pt
    },
    rampcombined inside/.style={
        rampcombined,
        draw=rampcombinedinside-draw,
        line width=3.2pt,
        minimum width=1.32cm+3.6pt,
        minimum height=5.4cm+3.6pt,
        rounded corners=0pt
    },
    turbocombined/.style={
        rectangle,
        draw=advanced-draw,
        thin,
        fill=advancedbutton-fill,
        inner sep=0cm,
        outer sep=0cm,
        minimum width=1.32cm+7.2pt,
        minimum height=1.2cm,
        rounded corners=0.25pt
    }
}

% for ramp text Memory, Ceiling, and Floor
\tikzset{
    ramptextconfig/.style={
        anyconfig,
        draw=none,
        fill=none,
        % with fill color: fill=ramptextconfig-fill,
        inner xsep=0.075cm,
        inner ysep=0.05cm
    }
}

% for Factory Reset
\tikzset{
    anyconfig factoryreset/.style={
        anyconfig,
        draw=factoryreset-draw,
        semithick, % semithick = 0.6pt
        fill=factoryreset-fill
    }
}

% for line from Factory Reset to Simple UI
\tikzset{
    anyline factoryreset simpleUI/.style={
        anyline,
        draw=factoryresetsimpleUI-draw,
        semithick,
        rounded corners=4.5pt
    }
}

% for general information
\tikzset{
    generalinfo/.style={
        anyconfig,
        draw=generalinfo-draw,
        fill=generalinfo-fill
    },
    andurilinfo/.style={
        generalinfo,
        draw=andurilinfo-draw,
        fill=andurilinfo-fill,
        text=andurilinfo-text
    },
    voltagecolorinfo/.style={
        generalinfo,
        fill=voltagecolorinfo-fill
    },
    buttonpressesinfo/.style={
        generalinfo,
        fill=buttonpressesinfo-fill
    },
    defaultsvaryinfo/.style={
        generalinfo,
        fill=defaultsvaryinfo-fill
    },
    matrixlegendinfo/.style={
        generalinfo,
        fill=matrixlegendinfo-fill,
        matrix of nodes,
        nodes in empty cells,
        nodes={
            rectangle,
            draw=none,
            inner sep=0cm,
            outer sep=0cm,
            minimum width=0.808cm,
            minimum height=0.285cm,
            sharp corners,
            anchor=west
        },
        row sep=0.088cm,
        column sep=0.225cm
    },
    usermanualinfo/.style={
        generalinfo,
        draw=usermanualinfo-draw,
        semithick,
        fill=usermanualinfo-fill
    }
}

% for legend
\tikzset{
    anyline basic use legend/.style={
        anyline basic use,
        line cap=round
    },
    anybutton basic use legend/.style={
        anybutton basic use,
        fill=matrixlegendinfo-fill
    },
    anyline legend/.style={
        anyline,
        line cap=round
    },
    anybutton legend/.style={
        anybutton,
        fill=matrixlegendinfo-fill
    },
    simpleline legend/.style={
        simpleline,
        line cap=round
    },
    simplebutton legend/.style={
        simplebutton,
        fill=matrixlegendinfo-fill,
        double=matrixlegendinfo-fill
    },
    advancedline legend/.style={
        advancedline,
        line cap=round
    },
    advancedbutton legend/.style={
        advancedbutton,
        fill=matrixlegendinfo-fill
    },
    anyline momentary legend/.style={
        anyline momentary,
        line cap=round
    }
}

\usepackage{tikz-layers} % for additional layers
% order of layers: background, behind, (main), above, glass

\usepackage{tikzpagenodes} % for additional current page nodes

% Preamble - 04 tabularray ---------------------------------------------
% ----------------------------------------------------------------------
\usepackage{tabularray} % for creating tables
% provides the tblr environment

\NewTblrEnviron{tblr-advancedconfigmenu} % for config menus
\SetTblrInner[tblr-advancedconfigmenu]{
    row{1}     = {abovesep=0.5pt}, % default: 2pt
    row{2-Y}   = {rowsep=0.5pt}, % default: 2pt
    row{Z}     = {abovesep=0.5pt, belowsep=0pt}, % default: 2pt
    row{2-Z}   = {font=\scriptsize},
    colsep     = 0pt, % default: 6pt
    column{1}  = {0.35cm},
    cell{1}{1} = {c=2}{},
    cells      = l % halign=l (left); default: j (justify)
}

\NewTblrEnviron{tblr-buttonpresses} % for button presses
\SetTblrInner[tblr-buttonpresses]{
    row{1}     = {abovesep=0.5pt},
    row{2-Y}   = {rowsep=0pt},
    row{3}     = {belowsep=0.9pt}, % for basic use fit
    row{Z}     = {abovesep=2pt, belowsep=0pt},
    colsep     = 0pt,
    column{3}  = {leftsep=2.25pt}, % for horizontal space to colon
    cell{1}{1} = {c=3}{font=\bfseries\normalsize},
    cell{Z}{1} = {c=3}{},
    cells      = l
}

% for no extra spacing on the right of a tblr environment
\newcommand{\spacingnoextra}{\\[-1pt]}

% Preamble - 05 tcolorbox ----------------------------------------------
% ----------------------------------------------------------------------
\usepackage{tcolorbox} % for colored and framed text boxes

\tcbuselibrary{skins} % for additional styles (skins)

% for setting options for every following tcolorbox
\tcbset{
    colframe=voltagebox-frame, % initially black!75!white
    opacityframe=0, % initially 1.0
    boxrule=0cm, % initially 0.05cm
    colback=voltagebox-back, % initially black!5!white
    coltext=voltagebox-text, % initially black
    left=0cm, % initially 0.4cm
    right=0cm, % initially 0.4cm
    top=0.01cm, % initially 0.2cm
    bottom=0.01cm, % initially 0.2cm
    boxsep=0cm, % initially 0.1cm
    arc=0.05cm, % initially 0.1cm
    sharp corners, % initially all rounded corners
    box align=base, % initially bottom
    nobeforeafter % for inside a tblr environment
}

% for the left part of voltage: red
\newtcbox{\voltageboxred}{
    colback=voltageboxcolor-red,
    left=0.025cm,
    rounded corners=west
}

% for the middle part of voltage: yellow, green, cyan, blue
\newtcbox{\voltagebox}[1][white]{
    colback=#1
}

% for the right part of voltage: purple
\newtcbox{\voltageboxpurple}{
    colback=voltageboxcolor-purple,
    right=0.025cm,
    rounded corners=east
}

% for the left part of voltage: red
% adv = Advanced UI function
\newtcbox{\voltageboxredadv}{
    colback=voltageboxcolor-red,
    coltext=voltageboxadv-text,
    left=0.025cm,
    rounded corners=west
}

% for the middle part of voltage: yellow, green, cyan, blue
% adv = Advanced UI function
\newtcbox{\voltageboxadv}[1][white]{
    colback=#1,
    coltext=voltageboxadv-text
}

% for the right part of voltage: purple
% adv = Advanced UI function
\newtcbox{\voltageboxpurpleadv}{
    colback=voltageboxcolor-purple,
    coltext=voltageboxadv-text,
    right=0.025cm,
    rounded corners=east
}

% for aux LED color for voltage
\newcommand{\textboxauxledcfv}{%
    \voltageboxred{R-2.9,%
        \vphantom{Voltage,Ú}}%
    \voltagebox[voltageboxcolor-yellow]{\hspace{0.01cm}Y-3.3,%
        \vphantom{Voltage,Ú}}%
    \voltagebox[voltageboxcolor-green]{\hspace{0.01cm}G-3.5,%
        \vphantom{Voltage,Ú}}%
    \voltagebox[voltageboxcolor-cyan]{\hspace{0.01cm}C-3.7,%
        \vphantom{Voltage,Ú}}%
    \voltagebox[voltageboxcolor-blue]{\hspace{0.01cm}B-3.9,%
        \vphantom{Voltage,Ú}}%
    \voltageboxpurple{\hspace{0.01cm}P-4.1V%
        \vphantom{Voltage,Ú}}%
}

% for voltage display in Off and Lockout modes,
% and for post-off voltage display
% adv = Advanced UI function
\newcommand{\textboxvoltageadv}{%
    \voltageboxredadv{V%
        \vphantom{Voltage}}%
    \voltageboxadv[voltageboxcolor-yellow]{o%
        \vphantom{Voltage}}%
    \voltageboxadv[voltageboxcolor-green]{lt%
        \vphantom{Voltage}}%
    \voltageboxadv[voltageboxcolor-cyan]{a%
        \vphantom{Voltage}}%
    \voltageboxadv[voltageboxcolor-blue]{g%
        \vphantom{Voltage}}%
    \voltageboxpurpleadv{e%
        \vphantom{Voltage}}%
}

% for tint ramp
\newtcbox{\tintrampbox}{
    colframe=tintrampbox-frame,
    skin=enhanced, % for interior style
    interior style={
        left color=tintrampboxcolor-left,
        right color=tintrampboxcolor-right
    },
    coltext=tintrampbox-text,
    left=0.025cm,
    right=0.025cm,
    rounded corners
}

% for tint ramp
% adv = Advanced UI function
\newtcbox{\tintrampboxadv}{
    colframe=tintrampbox-frame,
    skin=enhanced, % for interior style
    interior style={
        left color=tintrampboxcolor-left,
        right color=tintrampboxcolor-right
    },
    coltext=tintrampboxadv-text,
    left=0.025cm,
    right=0.025cm,
    rounded corners
}

% for smooth POVD rainbow colors
\pgfdeclarehorizontalshading{rainbow smooth POVD}{72pt}{
    color(0pt)=(voltageboxcolor-red);
    color(16pt)=(voltageboxcolor-red);
    color(24pt)=(voltageboxcolor-yellow);
    color(32pt)=(voltageboxcolor-green);
    color(40pt)=(voltageboxcolor-cyan);
    color(48pt)=(voltageboxcolor-blue);
    color(56pt)=(voltageboxcolor-purple);
    color(72pt)=(voltageboxcolor-purple)
}

% for smooth POVD
\newtcbox{\smoothPOVDbox}{
    colframe=smoothPOVDbox-frame,
    skin=enhanced, % for underlay
    underlay={
        \begin{tcbclipinterior}
            \shade [shading=rainbow smooth POVD]
            ([xshift=-1pt,yshift=1pt]interior.north west)
            rectangle
            ([xshift=1pt,yshift=-1pt]interior.south east);
        \end{tcbclipinterior}
    },
    coltext=smoothPOVDbox-text,
    left=0.025cm,
    right=0.025cm,
    top=0.015cm, % instead of 0.01cm
    bottom=0.025cm, % instead of 0.01cm
    rounded corners
}

% for smooth POVD
% adv = Advanced UI function
\newtcbox{\smoothPOVDboxadv}{
    colframe=smoothPOVDbox-frame,
    skin=enhanced, % for underlay
    underlay={
        \begin{tcbclipinterior}
            \shade [shading=rainbow smooth POVD]
            ([xshift=-1pt,yshift=1pt]interior.north west)
            rectangle
            ([xshift=1pt,yshift=-1pt]interior.south east);
        \end{tcbclipinterior}
    },
    coltext=smoothPOVDboxadv-text,
    left=0.025cm,
    right=0.025cm,
    top=0.015cm, % instead of 0.01cm
    bottom=0.025cm, % instead of 0.01cm
    rounded corners
}

% for HSV channel mode rainbow colors
\pgfdeclareverticalshading{rainbow HSV channel mode}{45pt}{
    color(0pt)=(voltageboxcolor-red);
    color(10pt)=(voltageboxcolor-red);
    color(15pt)=(voltageboxcolor-yellow);
    color(20pt)=(voltageboxcolor-green);
    color(25pt)=(voltageboxcolor-cyan);
    color(30pt)=(voltageboxcolor-blue);
    color(35pt)=(voltageboxcolor-purple);
    color(45pt)=(voltageboxcolor-purple)
}

% for HSV channel mode
% adv = Advanced UI function
\newtcbox{\HSVchannelmodeboxadv}{
    colframe=HSVchannelmodebox-frame,
    skin=enhanced, % for underlay
    underlay={
        \begin{tcbclipinterior}
            \shade [shading=rainbow HSV channel mode]
            ([xshift=-1.5pt,yshift=1.5pt]interior.north west)
            rectangle
            ([xshift=1.5pt,yshift=-1.5pt]interior.south east);
        \end{tcbclipinterior}
    },
    coltext=HSVchannelmodeboxadv-text,
    left=0.035cm, % instead of 0.025cm
    right=0.035cm, % instead of 0.025cm
    top=0.015cm, % instead of 0.01cm
    bottom=0.025cm, % instead of 0.01cm
    rounded corners
}

% Preamble - 06 Misc ---------------------------------------------------
% ----------------------------------------------------------------------

% for infotext for Simple UI and Advanced UI (any)
\newcommand{\infotextany}[1]{\textcolor{infoany-text}{#1}}

% for infotext for Advanced UI only
\newcommand{\infotextadvanced}[1]{\textcolor{infoadvanced-text}{#1}}

% for infotext for Advanced UI - config menus
\newcommand{\infotextadvancedconfigmenu}[1]{\textcolor
    {infoadvancedconfigmenu-text}{#1}}

% for infotext for Simple UI only
\newcommand{\infotextsimple}[1]{\textcolor{infosimple-text}{#1}}

% for text for basic use
\newcommand{\textbasicuse}[1]{\textcolor{basicuse-text}{#1}}

% for on lights with dimmable RGB aux LEDs
\newcommand{\dimmableRGB}{%
    \textcolor{dimmableRGB-color1}{\char"25CF}%
    \hspace{-0.05em}\textcolor{dimmableRGB-color2}{\char"25CF}%
    \hspace{-0.05em}\textcolor{dimmableRGB-color3}{\char"25CF}%
    % Unicode character hex code 25CF: Black Circle
}

% for text for inherited by Simple UI
\newcommand{\textinherited}[1]{\textcolor{inherited-text}{#1}}

% for inherited by Simple UI
\newcommand{\inherited}{\textcolor{inherited-color}{\char"002A}}
% Unicode character hex code 002A: Asterisk

% for the next aux LED setting
\newcommand{\nextaux}{\char"2192}
% Unicode character hex code 2192: Rightwards Arrow

% for the next aux LED setting again
\newcommand{\nextauxagain}{\char"2197}
% Unicode character hex code 2197: North East Arrow

% Body - Table of Contents ---------------------------------------------
% ----------------------------------------------------------------------

% 01 Ramp --------------------------------------------------------------
% 02 OFF ---------------------------------------------------------------
% 03 Lockout -----------------------------------------------------------
% 04 ON ----------------------------------------------------------------
% 05 OFF right ---------------------------------------------------------
% 06 Momentary Mode from OFF ON ----------------------------------------
% 07 Strobe & Mood Modes from OFF --------------------------------------
% 08 Blinky & Utility Modes from OFF -----------------------------------
% 09 Next Channel Mode from ON -----------------------------------------
% 10 Ramp Config from ON -----------------------------------------------
% 11 Simple UI Config from OFF -----------------------------------------
% 12 Momentary Moon from Lockout ---------------------------------------
% 13 Aux LED Color from OFF right Lockout ------------------------------
% 14 Tactical Mode from OFF right --------------------------------------
% 15 Factory Reset from OFF right --------------------------------------
% 16 Advanced UI switch from OFF right ---------------------------------
% 17 Anduril 2 UI ------------------------------------------------------

% 01 Ramp --------------------------------------------------------------
% ----------------------------------------------------------------------
\begin{document}

\begin{tikzpicture}[
    remember picture, % for referencing the current page nodes
    overlay, % for referencing the current page nodes
    font=\footnotesize
]

% Smooth Ramp
\node (Ramp Smooth)
at (current page.center)[
    rampsmooth,
    anchor=east,
    top color=rampsmooth-topbottom!0,
    bottom color=rampsmooth-topbottom!50
]
{};

% Stepped Ramp (number of steps: 7)
\node (Ramp Stepped 1-2)[
    alias=stepped-step,
    rampstepped,
    right=0cm of Ramp Smooth.south east,
    anchor=south west,
    top color=rampstepped-topbottom!60,
    bottom color=rampstepped-topbottom!50
]
{};

\foreach \x/\y/\z[
    evaluate=\y as \ytop using \y*10,
    evaluate=\z as \zbottom using \z*10
]
in {2-3/5/4, 3-4/4/3, 4-5/3/2, 5-6/2/1, 6-7/1/0}
\node (Ramp Stepped \x)[
    alias=stepped-step,
    rampstepped,
    above=0cm of stepped-step,
    anchor=south,
    top color=rampstepped-topbottom!\ytop,
    bottom color=rampstepped-topbottom!\zbottom
]
{};

% Further ramp elements
\node (Ramp inside)
at (current page.center)[
    rampcombined inside
]
{};

\node (Ramp)
at (current page.center)[
    rampcombined
]
{};

\node (ON Ramp)
at (Ramp.center)[
    on-rampimage
]
{ON};

\node (Ramp text Memory)
at (ON Ramp.south)[
    ramptextconfig,
    below=0.05cm
]
{\oneline{\infotextany{Memory}}};

\node (Ramp text Ceiling)
at (Ramp.north)[
    ramptextconfig,
    below=0.05cm+3.2pt % +3.2pt for Ramp inside
]
{\oneline{\infotextany{Ceiling}}};

\node (Ramp text Floor)
at (Ramp.south)[
    ramptextconfig,
    above=3.2pt % +3.2pt for Ramp inside
    % with fill color: above=0.05cm+3.2pt
]
{\oneline{\infotextany{Floor\vphantom{y}}}};

\node (Ramp Style Smooth)[
    below right=0.05cm and 0.36cm of Ramp.south west,
    anchor=north,
    align=center
]
{\infotextadvanced{Smooth}};

\node (Ramp Style Stepped)[
    below left=0.05cm and 0.36cm+0.02cm of Ramp.south east,
    % +0.02cm for vertical alignment
    anchor=north,
    align=center
]
{\infotextadvanced{Stepped}};

\node (Ramp Styles fit)[
    inner sep=-0.05cm,
    fit=(Ramp Style Smooth)
    (Ramp Style Stepped)
]
{};

\path (ON Ramp.north) edge[
    {<[fill=fillarrowbasicuse]}->,
    anyline basic use,
    shorten <=0.15cm, % out
    shorten >=0.05cm % in
]
node[
    anybutton basic use,
    pos=0.53,
    label={ [align=left, xshift=0.515cm] right:
        \infotextany{Ramp up}\\
        \infotextany{(floor\textendash{}ceiling),}\\
        \infotextany{with reversal}\\
        \infotextany{if released}\\
        \infotextany{and held again}\\
        \infotextany{within 1s}
    }
]
{1H} (Ramp text Ceiling.south);

\path (Ramp text Memory.south) edge[
    ->,
    anyline ramp,
    shorten <=0.06cm, % out
    shorten >=0.05cm % in
]
node[
    anybutton,
    pos=0.43,
    label={ [align=left, xshift=0.515cm+1.4pt] right:
    % +1.4pt for vertical alignment
        \infotextany{Ramp down}\\
        \infotextany{(ceiling\textendash{}floor)}
    }
]
{2H} (Ramp text Floor.north);

% Turbo
{ [on background layer]
    \node (Turbo)[
        turbocombined,
        below=0.01pt of Ramp.north, % 0.01pt for a cleaner look
        anchor=south
    ]
    {};
}

\node (Turbo text)
at (Turbo.north)[
    below,
    fill=none
]
{\oneline{\infotextadvanced{Turbo}}};

\node (Ramp label)[
    on-extra ramp label,
    above=0.2cm of Turbo.north,
    anchor=south
]
{R\hspace{0.1em}A\hspace{0.1em}M\hspace{0.1em}P};

% Thermal regulation information
\node (Thermal regulation information)[
    above=0.05cm of Ramp label.north,
    anchor=south,
    inner xsep=0.05cm,
    inner ysep=0cm,
    align=left
]
{
    \infotextany{Thermal regulation}\\
    \infotextany{only in continuous}\\
    \infotextany{ramp mode}\\
    \infotextany{(up to turbo)}
};

% Toggle Ramp Style from ON
\node (Toggle Ramp Style from ON)[
    advancedconfig,
    below=0.8cm of Ramp.south,
    anchor=north,
    align=center
]
{\oneline{Toggle Ramp Style\inherited{}}};

\node (Toggle Ramp Style extra node ON)[
    on-extra toggle ramp style,
    below right=0.9cm and 0.2cm of Toggle Ramp Style from ON.west,
    anchor=center
]
{ON};

\path (Toggle Ramp Style extra node ON.east) edge[
    ->,
    advancedline,
    out=0,
    in=270,
    out looseness=0.8,
    in looseness=1.8
]
node[
    advancedbutton,
    pos=0.6,
    label={ [align=left] right:
        \infotextadvanced{3C if only one}\\
        \infotextadvanced{channel mode}
    }
]
{6C} (Toggle Ramp Style from ON.south);

\path (Toggle Ramp Style from ON.north) edge[
    ->,
    advancedline,
    out=90,
    in=225,
    out looseness=0.5,
    in looseness=2
]
(Ramp Styles fit.335);

\path (Toggle Ramp Style from ON.north) edge[
    ->,
    advancedline,
    out=90,
    in=315,
    out looseness=0.5,
    in looseness=2
]
(Ramp Styles fit.205);

% 02 OFF ---------------------------------------------------------------
% ----------------------------------------------------------------------
{ [on glass layer]
    \node (OFF)[
        onoff,
        left=3.6cm of Ramp
    ]
    {OFF};
}

\path (OFF.north) edge[
    ->,
    advancedline momentary,
    shorten >=0.2pt, % in
    % 0.2pt: 0.5 line thickness (turbocombined)
    out=90,
    in=180,
    looseness=1.4
]
node[
    advancedbutton,
    pos=0.818
]
{2H} (Turbo.north west);

{ [on above layer]
    \path (OFF.north) edge[
        ->,
        anyline,
        shorten >=0.2pt, % in
        % 0.2pt: 0.5 line thickness (rampcombined)
        out=90,
        in=180,
        looseness=1.3
    ]
    node[
        anybutton,
        pos=0.7
    ]
    {2C}
    (Ramp.north west);
}

\path (OFF.21) edge[
    -, % no extra arrow tip
    simpleline momentary,
    shorten <=-0.1pt, % out
    shorten >=2pt, % in
    out=90,
    in=180,
    out looseness=1.3,
    in looseness=0.67
]
node[
    simplebutton,
    pos=0.18
]
{2H} (Ramp.north west);

% Advanced UI: Anduril 2 turbo style
\path (Ramp.north) edge[
    {Round Cap[sep=0.7cm]}-{Round Cap[sep=2.3cm]},
    advancedline,
    advancedbutton-fill,
    line width=2.4pt,
    shorten <=0.7pt, % out
    shorten >=0.2pt, % in
    out=40,
    in=0,
    out looseness=0.3,
    in looseness=3
]
node
{} (Turbo.north east);
\path (Ramp.north) edge[
    <->,
    advancedline,
    shorten <=0.7pt, % out
    shorten >=0.2pt, % in
    out=40,
    in=0,
    out looseness=0.3,
    in looseness=3
]
node[
    advancedbutton,
    pos=0.6,
    label={ [align=left] right:
        \infotextadvanced{If ramped up}\\
        \infotextadvanced{to ceiling first}\\
        \infotextadvanced{(Andúril 2 style)}
    }
]
{2C} (Turbo.north east);

\path (Ramp.west) edge[
    {<[scale width=0.75]}-{>[scale width=0.75]},
    anyline,
    shorten <=0.8pt, % out
    shorten >=0.5pt, % in
    out=25,
    in=144,
    relative,
    looseness=1.16
]
node[
    anybutton,
    pos=0.57
]
{2C} (Ramp.north west);

% Momentary Turbo 3H from ON, ramp image
{ [on above layer]
    \path (Ramp.west) edge[
        {Round Cap[sep=3.35cm]}-{Round Cap[sep=1.3cm]},
        advancedline,
        advancedbutton-fill,
        line width=2.4pt,
        shorten <=2pt, % out
        shorten >=0.5pt, % in
        out=54,
        in=140,
        relative,
        looseness=1.3
    ]
    node
    {} (Turbo.north west);
    \path (Ramp.west) edge[
        -{>[scale width=0.75]},
        advancedline momentary,
        shorten <=2pt, % out
        shorten >=0.5pt, % in
        out=54,
        in=140,
        relative,
        looseness=1.3
    ]
    node[
        advancedbutton,
        pos=0.852
    ]
    {3H} (Turbo.north west);
}

{ [on above layer]
    \path (OFF.south) edge[
        ->,
        anyline,
        shorten >=0.2pt, % in
        out=270,
        in=180,
        looseness=1.3
    ]
    node[
        anybutton,
        pos=0.7
    ]
    {1H} (Ramp.south west);
}

\path (OFF.east) edge[
    <->,
    anyline basic use,
    shorten >=0.2pt % in
]
node[
    anybutton basic use,
    pos=0.5
]
{1C} (Ramp.west);

% 03 Lockout -----------------------------------------------------------
% ----------------------------------------------------------------------
{ [on glass layer]
    \node (Lockout)[
        onoff,
        right=3.6cm of Ramp
    ]
    {LOCK\footnotesize OUT};
}

{ [on above layer]
    \path (Lockout.north) edge[
        ->,
        anyline,
        shorten >=0.2pt, % in
        out=90,
        in=0,
        looseness=1.3
    ]
    node[
        anybutton,
        pos=0.7
    ]
    {5C} (Ramp.north east);
}

\path (Lockout.south) edge[
    ->,
    anyline,
    shorten >=0.2pt, % in
    out=270,
    in=0,
    looseness=1.3
]
node[
    anybutton,
    pos=0.7
]
{4H} (Ramp.south east);

\path (Lockout.west) edge[
    <->,
    anyline,
    shorten >=0.2pt % in
]
node[
    anybutton,
    pos=0.5
]
{4C} (Ramp.east);

% 04 ON ----------------------------------------------------------------
% ----------------------------------------------------------------------
{ [on glass layer]
    \node (ON)[
        onoff,
        left=4.6cm of OFF
    ]
    {ON};
}

\path (ON.east) edge[
    <->,
    anyline
]
node[
    anybutton,
    pos=0.5
]
{1C} (OFF.west);

% 05 OFF right ---------------------------------------------------------
% ----------------------------------------------------------------------
{ [on glass layer]
    \node (OFF right)[
        onoff,
        right=6.6cm of Lockout
    ]
    {OFF};
}

\path (Lockout.east) edge[
    <->,
    anyline
]
node (Lockout to OFF right)[
    anybutton,
    pos=0.4
]
{3C}
node (OFF right to Lockout)[
    anybutton,
    pos=0.6
]
{4C} (OFF right.west);

\node (Lockout to OFF right arrow)[
    lockoutoffright-arrow,
    above=0.1cm of Lockout to OFF right.north,
    anchor=south
]
{};

\path (Lockout to OFF right arrow.south west) edge[
    ->,
    anyline,
    line cap=round
]
node
{} (Lockout to OFF right arrow.south east);

\node (OFF right to Lockout arrow)[
    lockoutoffright-arrow,
    above=0.1cm of OFF right to Lockout.north,
    anchor=south
]
{};

\path (OFF right to Lockout arrow.south east) edge[
    ->,
    anyline,
    line cap=round
]
node
{} (OFF right to Lockout arrow.south west);

\node (OFF right extra node ON)[
    on-extra,
    below left=0.75cm and 1.1cm of OFF right.west,
    anchor=center
]
{ON};

\path (OFF right extra node ON.east) edge[
    <->,
    anyline,
    out=0,
    in=270,
    looseness=1
]
node[
    anybutton,
    pos=0.45
]
{1C} (OFF right.210);

% 06 Momentary Mode from OFF ON ----------------------------------------
% ----------------------------------------------------------------------
\node (Momentary Mode from OFF ON)[
    advancedconfig,
    below left=1.1cm and 2.3cm of OFF.center,
    anchor=north
]
{
    \firstline{Momentary Mode}
    Exit: disconnect power
};

\path (OFF.south) edge[
    ->,
    advancedline,
    out=270,
    in=0,
    looseness=1
]
node[
    advancedbutton,
    pos=0.65
]
{5C} (Momentary Mode from OFF ON.1);

\path (ON.south) edge[
    ->,
    advancedline,
    out=270,
    in=180,
    looseness=1
]
node[
    advancedbutton,
    pos=0.65
]
{5C} (Momentary Mode from OFF ON.179);

% 07 Strobe & Mood Modes from OFF --------------------------------------
% ----------------------------------------------------------------------

% Lightning Storm from Police Strobe
\node (Lightning Storm from Police Strobe)
at (current page text area.south west)[
    advancedconfig-mode,
    xshift=\borderspace,
    yshift=4.15cm,
    anchor=south west
]
{\oneline{Lightning Storm}};

% Police Strobe from Tactical Strobe
\node (Police Strobe from Tactical Strobe)[
    advancedconfig-mode,
    below=0.8cm of Lightning Storm from Police Strobe.south,
    anchor=north
]
{\oneline{Police Strobe}};

\path (Police Strobe from Tactical Strobe.north) edge[
    ->,
    advancedline-mode,
    out=90,
    in=270,
    looseness=1
]
node[
    advancedbutton-mode,
    pos=0.4
]
{2C} (Lightning Storm from Police Strobe.south);

% Tactical Strobe from Party Strobe
\node (Tactical Strobe from Party Strobe)[
    advancedconfig-mode,
    below=0.8cm of Police Strobe from Tactical Strobe.south,
    anchor=north
]
{\oneline{Tactical Strobe}};

\path (Tactical Strobe from Party Strobe.north) edge[
    ->,
    advancedline-mode,
    out=90,
    in=270,
    looseness=1
]
node[
    advancedbutton-mode,
    pos=0.4
]
{2C} (Police Strobe from Tactical Strobe.south);

% Party Strobe from Bike Flasher
\node (Party Strobe from Bike Flasher)[
    advancedconfig-mode,
    below=0.8cm of Tactical Strobe from Party Strobe.south,
    anchor=north
]
{\oneline{Party Strobe}};

\path (Party Strobe from Bike Flasher.north) edge[
    ->,
    advancedline-mode,
    out=90,
    in=270,
    looseness=1
]
node[
    advancedbutton-mode,
    pos=0.4
]
{2C} (Tactical Strobe from Party Strobe.south);

% Candle from OFF
\node (Candle from OFF)[
    advancedconfig-mode,
    above right=0.55cm and 1.4cm of
    Lightning Storm from Police Strobe.east,
    anchor=west
]
{\oneline{Candle}};

\path (Lightning Storm from Police Strobe.east) edge[
    ->,
    advancedline-mode,
    out=0,
    in=180,
    looseness=1
]
node[
    advancedbutton-mode,
    pos=0.4
]
{2C} (Candle from OFF.west);

% Candle Timer
\node (Candle Timer)[
    advancedconfig,
    above left=0.5cm and 1.2cm of Candle from OFF.west,
    anchor=east
]
{
    \firstline{Candle Timer}
    1 blink = 5 minutes
};

{ [on behind layer]
    \path (Candle from OFF.north west) edge[
        ->,
        advancedline,
        shorten <=-2.1pt, % out
        out=135,
        in=0,
        looseness=0.7
    ]
    node[
        advancedbutton,
        pos=0.45
    ]
    {5H} (Candle Timer.east);
}

% Bike Flasher from Candle
\node (Bike Flasher from Candle)[
    advancedconfig-mode,
    below=0.8cm of Candle from OFF.south,
    anchor=north
]
{\oneline{Bike Flasher}};

\path (Candle from OFF.south) edge[
    ->,
    advancedline-mode,
    out=270,
    in=90,
    looseness=1
]
node[
    advancedbutton-mode,
    pos=0.4
]
{2C} (Bike Flasher from Candle.north);

\path (Bike Flasher from Candle.west) edge[
    ->,
    advancedline-mode,
    out=180,
    in=0,
    looseness=0.9
]
node[
    advancedbutton-mode,
    pos=0.48
]
{2C} (Party Strobe from Bike Flasher.east);

% Strobe & Mood Modes Actions
\node (Strobe Modes Actions)
at (current page text area.south)[
    advancedconfig,
    xshift=-8.2cm,
    yshift=\borderspace,
    anchor=south east
]
{
    \firstline{Actions}
    1H/2H = Brighter/dimmer\\
    (Candle and Bike)\\
    1H/2H = Faster/slower\\
    (Party and Tactical)\\
    3C = Next channel mode\\
    (except Police),\\
    saved per strobe mode\\
    4C = Previous strobe mode\vphantom{y}
};

% Strobe Modes border

% +0.2pt: 0.5 line thickness for outer alignment
\coordinate (Strobe Modes border 1) at
    ($(Candle Timer.north east)
    + (0cm, \borderspace + 0.2pt)$); % yshift=0.2pt
\coordinate (Strobe Modes border 2) at
    ($(Strobe Modes Actions.south east)
    + (\borderspace + 0.2pt, -\borderspace + 0.2pt)$);
    % xshift=0.2pt, yshift=0.2pt
\coordinate (Strobe Modes border 3) at
    ($(Lightning Storm from Police Strobe.north west)
    + (-\borderspace + 0.2pt, 0cm)$); % xshift=0.2pt
\coordinate (Strobe Modes border 4) at
    ($(Candle Timer.north west)
    + (0cm, \borderspace + 0.2pt)$); % yshift=0.2pt

{ [on behind layer]
    \path[
        advancedborder-mode
    ]
    (Strobe Modes border 1)
    -| (Strobe Modes border 2)
    -| (Strobe Modes border 3)
    |- (Strobe Modes border 4)
    -- cycle;
}

% +0.2pt: 0.5 line thickness for outer alignment
\coordinate (Strobe Modes border north-east) at
    ({$(Strobe Modes border 1) + (0cm, 0.2pt)$}
    -| {$(Strobe Modes border 2) + (0.2pt, 0cm)$});

\coordinate (Strobe Modes border mode-memory) at
    ($(Strobe Modes border north-east) + (-0.327cm, 0cm)$);

\coordinate (Strobe Modes border momentary-strobe) at
    ($(Strobe Modes border north-east) + (-2.475cm, 0cm)$);

\node (Strobe Modes border title)[
    below=0cm of Strobe Modes border north-east,
    anchor=north east,
    inner xsep=0.15cm,
    inner ysep=0.1cm,
    align=right
]
{
    \normalsize\textbf{\infotextadvanced{Strobe \& Mood Modes}}
    \spacingv
    \infotextadvanced{1C = Off}
};

\path[
    -,
    advancedline
]
(OFF.south) to[
    out=270,
    in=10,
    looseness=1.3
]
+(-1.7cm, -2.15cm) edge[
    ->,
    advancedline,
    out=190,
    in=75,
    looseness=1,
    postaction={
        decoration={
            text along path,
            text={|\footnotesize\color{infoadvanced-text}|Mode memory},
            reverse path,
            text align={left indent=1.3cm},
            raise=0.08cm
        },
        decorate
    }
]
node[
    advancedbutton,
    pos=0
]
{3H} (Strobe Modes border mode-memory);

{ [on background layer]
    \path (Strobe Modes border momentary-strobe) edge[
        ->,
        advancedline,
        shorten <=-0.1pt, % out
        out=80,
        in=180,
        looseness=0.9
    ]
    node[
        advancedbutton,
        pos=0.2,
        label={ [align=left] right:
            \infotextadvanced{Momentary}\\
            \infotextadvanced{Strobe Mode}
        }
    ]
    {5C} (Momentary Mode from OFF ON.190);
}

% 08 Blinky & Utility Modes from OFF -----------------------------------
% ----------------------------------------------------------------------

% Next Channel Mode from Batt Check
\node (Next Channel Mode from Batt Check)
at (current page text area.south)[
    advancedconfig,
    xshift=-2.5cm,
    yshift=\borderspace,
    anchor=south west
]
{
    \firstline{Next Channel Mode\inherited{}}
    For number blinks only
};

% Batt Check from OFF
\node (Batt Check from OFF)[
    anyconfig-mode,
    above left=1.62cm and 0.51cm of
    Next Channel Mode from Batt Check.north west,
    anchor=south west
]
{
    \firstline{Batt Check}
    0.02V steps
};

% Batt Check Simple Advanced
\node (Batt Check Simple Advanced)[
    above right=0.05cm and 1.1cm of Batt Check from OFF.north west,
    anchor=south west,
    inner xsep=0.05cm,
    inner ysep=0cm,
    align=left
]
{
    \infotextsimple{Simple UI: voltage displayed only once}\\
    \infotextadvanced{Advanced UI: voltage displayed repeatedly}
};

\path (Batt Check from OFF.213) edge[
    ->,
    advancedline,
    shorten >=-2.01pt, % in
    out=270,
    in=120,
    looseness=0.9
]
node[
    advancedbutton,
    pos=0.5
]
{3C} (Next Channel Mode from Batt Check.north west);

{ [on above layer]
    \path (OFF.south) edge[
        {Round Cap[sep=2.85cm]}-{Round Cap[sep=3.75cm]},
        anyline,
        anybutton-fill,
        line width=2.4pt,
        out=270,
        in=120,
        looseness=1
    ]
    node
    {} (Batt Check from OFF.85);
    \path (OFF.south) edge[
        ->,
        anyline,
        out=270,
        in=120,
        looseness=1
    ]
    node[
        anybutton,
        pos=0.8
    ]
    {3C} (Batt Check from OFF.85);
}

% Voltage Config from Batt Check
{ [on background layer]
    \node (Voltage Config from Batt Check)[
        advancedconfigmenu,
        right=1.8cm of Strobe Modes Actions.south east,
        anchor=south west
    ]
    {
        \spacingnoextra
        \begin{tblr-advancedconfigmenu}{
            row{5} = {abovesep=0pt}
        }
            Voltage Config\inherited{} &\\
            1 & Voltage Correction Factor\\
              & {\char"00B1{}0.30V, 0.05V steps\\
              % Unicode character hex code 00B1: Plus-Minus Sign
                 1C = \char"2212{}0.30V\\
                 % Unicode character hex code 2212: Minus Sign
                 7C = 0V (default)\\
                 13C = +0.30V}\\
            2 & Post-Off \textboxvoltageadv{} Display\\
              & POVD, or \smoothPOVDboxadv{Smooth POVD}\\
              & {0C = Disable\\
                 nC = n seconds;\\
                 brightness as set in 3 \& 4,\\
                 high if high in Off \char"2192{} 7C,\\
                 % Unicode character hex code 2192: Rightwards Arrow
                 ramps if Smooth POVD}\\
            3 & Aux low ramp level\\
              & {0C = Disable button LEDs\\
                 1+C = Light up button\\
                 LEDs at this ramp level}\\
            4 & Aux high ramp level\\
              & {0C = Disable brighter\\
                 button LEDs\\
                 1+C = Brighter button\\
                 LEDs at this ramp level}\\
            5 & Aux while On\\
              & {0C = None\\
                 1C = Single-color only\\
                 2C = RGB only (see POVD)\\
                 3C = Both}\\
        \end{tblr-advancedconfigmenu}
    };
}

\path (Batt Check from OFF.west) edge[
    ->,
    advancedline,
    out=180,
    in=30,
    looseness=1
]
node[
    advancedbutton,
    pos=0.5
]
{7H} (Voltage Config from Batt Check.312);

% Temp Check from Batt Check
\node (Temp Check from Batt Check)[
    advancedconfig-mode,
    below right=0.2cm and 1.6cm of Batt Check from OFF.east,
    anchor=west
]
{\oneline{Temp Check}};

\path (Batt Check from OFF.east) edge[
    ->,
    advancedline-mode,
    out=0,
    in=180,
    looseness=1
]
node[
    advancedbutton-mode,
    pos=0.5
]
{2C} (Temp Check from Batt Check.west);

% Temp Check Thermal Config
{ [on background layer]
    \node (Temp Check Thermal Config)[
        advancedconfigmenu,
        above right=0.2cm and 1cm of
        Temp Check from Batt Check.north east,
        anchor=north west
    ]
    {
        \spacingnoextra
        \begin{tblr-advancedconfigmenu}{}
            Thermal Config\inherited{} &\\
            1 & Calibrate\\
              & nC = n\,°C\\
            2 & Limit\\
              & nC = n + 30\,°C\\
        \end{tblr-advancedconfigmenu}
    };
}

\path (Temp Check from Batt Check.east) edge[
    ->,
    advancedline,
    out=0,
    in=180,
    looseness=1
]
node[
    advancedbutton,
    pos=0.4
]
{7H} (Temp Check Thermal Config.186);

% Beacon from Temp Check
\node (Beacon from Temp Check)[
    advancedconfig-mode,
    below left=1cm and 0cm of Temp Check from Batt Check.south,
    anchor=north
]
{\oneline{Beacon}};

\path (Temp Check from Batt Check.220) edge[
    ->,
    advancedline-mode,
    out=270,
    in=90,
    looseness=1
]
node[
    advancedbutton-mode,
    pos=0.4
]
{2C} (Beacon from Temp Check.55);

% Beacon Speed
\node (Beacon Speed)[
    advancedconfig,
    below right=0.68cm and 1cm of Beacon from Temp Check.east,
    anchor=west
]
{
    \firstline{Beacon Speed}
    1 blink = 1 second pause
};

\path (Beacon from Temp Check.east) edge[
    ->,
    advancedline,
    out=0,
    in=180,
    looseness=1
]
node[
    advancedbutton,
    pos=0.4
]
{1H} (Beacon Speed.west);

% SOS from Beacon
\node (SOS from Beacon)[
    advancedconfig-mode,
    above left=0.2cm and 1.8cm of Beacon from Temp Check.west,
    anchor=east
]
{\oneline{SOS}};

\path (Beacon from Temp Check.west) edge[
    ->,
    advancedline-mode,
    out=180,
    in=0,
    looseness=1
]
node[
    advancedbutton-mode,
    pos=0.5
]
{2C} (SOS from Beacon.east);

\path (SOS from Beacon.north) edge[
    ->,
    advancedline-mode,
    out=90,
    in=270,
    looseness=1
]
node[
    advancedbutton-mode,
    pos=0.4
]
{2C} (Batt Check from OFF.south);

% Blinky Modes border

% +0.2pt: 0.5 line thickness for outer alignment
\coordinate (Blinky Modes border 1) at
    ($(Voltage Config from Batt Check.north east)
    + (0.9cm, \borderspace)$);
\coordinate (Blinky Modes border 2) at
    ($(Batt Check Simple Advanced.north west)
    + (0.2cm, \borderspace + 0.2pt)$); % yshift=0.2pt
\coordinate (Blinky Modes border 3) at
    ($(Beacon Speed.north east)
    + (\borderspace, 0cm)$);
\coordinate (Blinky Modes border 4) at
    ($(Voltage Config from Batt Check.south west)
    + (-\borderspace, -\borderspace + 0.2pt)$); % yshift=0.2pt
\coordinate (Blinky Modes border 5) at
    ($(Voltage Config from Batt Check.north west)
    + (-\borderspace, \borderspace)$);

{ [on behind layer]
    \path[
        advancedborder-mode
    ]
    (Blinky Modes border 1)
    -- (Blinky Modes border 2)
    -| (Blinky Modes border 3)
    |- (Blinky Modes border 4)
    -- (Blinky Modes border 5)
    -- cycle;
}

% +/-0.2pt: 0.5 line thickness for outer alignment
\coordinate (Blinky Modes border north-east) at
    ({$(Blinky Modes border 2) + (0cm, 0.2pt)$}
    -| {$(Blinky Modes border 3) + (0.2pt, 0cm)$});
\coordinate (Blinky Modes border south-east) at
    ({$(Blinky Modes border 4) + (0cm, -0.2pt)$}
    -| {$(Blinky Modes border 3) + (0.2pt, 0cm)$});

\node (Blinky Modes border title)[
    below=0cm of Blinky Modes border north-east,
    anchor=north east,
    inner xsep=0.15cm,
    inner ysep=0.1cm,
    align=right
]
{
    \normalsize\textbf{\infotextadvanced{Blinky \& Utility Modes}}
    \spacingv
    \infotextadvanced{1C = Off}
};

% 09 Next Channel Mode from ON -----------------------------------------
% ----------------------------------------------------------------------
\node (Next Channel Mode from ON)
at (current page text area.west)[
    anyconfig,
    yshift=0.4cm,
    anchor=west
]
{\oneline{Next Channel Mode}};

{ [on above layer]
    \path (ON.west) edge[
        ->,
        anyline,
        out=180,
        in=0,
        looseness=1
    ]
    node[
        anybutton,
        pos=0.64
    ]
    {3C} (Next Channel Mode from ON.east);
}

% Tint Ramping from ON
{ [on background layer]
    \node (Tint Ramping from ON)[
        anyconfig,
        below=\nodespacev of Next Channel Mode from ON.south west,
        anchor=north west
    ]
    {
        \firstline{\tintrampbox{Tint Ramp}}
        If this channel mode can
    };
}

{ [on above layer]
    \path (ON.west) edge[
        ->,
        anyline,
        out=180,
        in=0,
        looseness=1
    ]
    node[
        anybutton,
        pos=0.6
    ]
    {3H} (Tint Ramping from ON.east);
}

% Momentary Turbo 3H from ON
\node (Momentary Turbo 3H from ON)[
    advancedconfig,
    below=\nodespacev of Tint Ramping from ON.south west,
    anchor=north west
]
{
    \firstline{Momentary Turbo}
    If no tint ramping
};

\path (ON.west) edge[
    ->,
    advancedline momentary,
    out=180,
    in=0,
    looseness=0.8
]
node[
    advancedbutton,
    pos=0.652
]
{3H} (Momentary Turbo 3H from ON.east);

% Momentary Turbo 4H from ON
\node (Momentary Turbo 4H from ON)[
    advancedconfig,
    below=\nodespacev of Momentary Turbo 3H from ON.south west,
    anchor=north west
]
{
    \firstline{Momentary Turbo}
    On multi-channel lights
};

\path (ON.west) edge[
    ->,
    advancedline momentary,
    out=180,
    in=0,
    looseness=0.8
]
node[
    advancedbutton,
    pos=0.71
]
{4H} (Momentary Turbo 4H from ON.east);

% Sunset Timer from ON
\node (Sunset Timer from ON)[
    advancedconfig,
    above right=1.2cm and 2.3cm of ON.center,
    anchor=south
]
{
    \firstline{Sunset Timer}
    1 blink = 5 minutes
};

\path (ON.north) edge[
    ->,
    advancedline,
    out=90,
    in=180,
    looseness=1
]
node[
    advancedbutton,
    pos=0.65
]
{5H} (Sunset Timer from ON.west);

% 10 Ramp Config from ON -----------------------------------------------
% ----------------------------------------------------------------------
{ [on background layer]
    \node (Ramp Config from ON)
    at (current page text area.north west)[
        advancedconfigmenu,
        anchor=north west
    ]
    {
        \spacingnoextra
        \begin{tblr-advancedconfigmenu}{
            row{2,6}     = {abovesep=3pt, belowsep=2pt},
            cell{2,6}{1} = {c=2}{}
        }
            Ramp Config &\\
            Smooth Ramp &\\
            1 & Floor: nC = n of 150\\
            2 & Ceiling: nC = 151 \char"2212{} n\\
            % Unicode character hex code 2212: Minus Sign
            3 & {Speed\inherited{}: 1C (full speed)\\
                 to 4C (quarter speed)}\\
            Stepped Ramp &\\
            1 & Floor: nC = n of 150\\
            2 & Ceiling: nC = 151 \char"2212{} n\\
            % Unicode character hex code 2212: Minus Sign
            3 & Step count: 1\textendash{}150C\\
        \end{tblr-advancedconfigmenu}
    };
}

\path[
    ->,
    advancedline
]
(ON.north) to[
    out=90,
    in=280,
    looseness=1.4
]
+(-3.57cm, 3.75cm) to[
    out=100,
    in=275,
    looseness=0.9
]
node[
    advancedbutton,
    pos=0.15
]
{7H}
+(-3.68cm, 4.9cm) to[
    out=95,
    in=320,
    looseness=0.9
]
(Ramp Config from ON.295);

% Channel Mode Config from ON
{ [on background layer]
    \node (Channel Mode Config from ON)[
        advancedconfigmenu,
        above=0.75cm of Next Channel Mode from ON.north west,
        anchor=south west
    ]
    {
        \spacingnoextra
        \begin{tblr-advancedconfigmenu}{
            row{4-5}     = {abovesep=3pt, belowsep=2pt},
            cell{4-5}{1} = {c=2}{}
        }
            {Channel Mode\\
             Config\inherited{}} &\\
            n & n\textsuperscript{th} channel mode\\
            & {0C = Disable\\
               1C = Enable}\\
            + 1 \HSVchannelmodeboxadv{HSV} channel \dimmableRGB{}\\
            {Typical sequence for\\
             dual-channel lights}\\
            1  & Channel 1\\
            2  & Channel 2\\
            3  & 50/50\\
            4  & \tintrampboxadv{Tint Ramp}\\
            5  & {Auto-tint\\
                  (3H = Reverse)}\\
            6+ & Aux LED channels\\
        \end{tblr-advancedconfigmenu}
    };
}

\path (ON.north) edge[
    ->,
    advancedline,
    out=90,
    in=315,
    looseness=1.2
]
node[
    advancedbutton,
    pos=0.68,
    label={ [align=left] below:
        \infotextadvanced{9H from any mode}\\
        \infotextadvanced{unless overridden}
   }
]
{9H} (Channel Mode Config from ON.318);

% Ramp Extras Config from ON
{ [on background layer]
    \node (Ramp Extras Config from ON)[
        advancedconfigmenu,
        right=\nodespaceh of Ramp Config from ON.north east,
        anchor=north west
    ]
    {
        \spacingnoextra
        \begin{tblr-advancedconfigmenu}{}
            Ramp Extras Config &\\
            1 & Disable manual memory\inherited{}\\
              & 1+C\\
            2 & Manual memory timer\inherited{}\\
              & {0C = Disable\\
                 nC = n minutes}\\
            3 & {Ramp-after-moon style\inherited{}\\
                 (1H from Off)}\\
              & {0C = Ramp up after moon\\
                 1C = Stay at floor level}\\
            4 & {Advanced UI's turbo style\\
                 (2C from On/Ramp)}\\
              & {0C = Ceiling only, no turbo\\
                 \char"2192{} momentary ceiling only\\
                 % Unicode character hex code 2192: Rightwards Arrow
                 1C = Turbo\\
                 (Andúril 1 style)\\
                 2C = Ceiling, or turbo if\\
                 ramped up to ceiling first\\
                 (Andúril 2 style)}\\
            5 & Smooth Steps\inherited{}\\
              & {0C = Disable\\
                 1C = Enable}\\
        \end{tblr-advancedconfigmenu}
    };
}

\path (ON.north) edge[
    ->,
    advancedline,
    out=90,
    in=270,
    looseness=1.5
]
node[
    advancedbutton,
    pos=0.8
]
{10H} (Ramp Extras Config from ON.275);

% Types of brightness memory information
\node (Types of brightness memory information)[
    advancedconfig,
    right=0.3cm of Ramp Extras Config from ON.south east,
    anchor=south west
]
{
    Types of brightness\\
    memory (1C from Off)
    \spacingv
    Automatic: last-ramped\\
    Manual: user's saved\\
    Hybrid: automatic first,\\
    manual after n minutes\vphantom{y}
};

% Manual Memory from ON
\node (Manual Memory from ON)[
    advancedconfig,
    below=0.2cm of Types of brightness memory information.south west,
    anchor=north west
]
{
    \firstline{Manual Memory\inherited{}}
    Activate, and save\\
    current brightness and\\
    current channel mode\vphantom{y}
};

\path (ON.north) edge[
    ->,
    advancedline,
    out=90,
    in=270,
    looseness=1.5
]
node[
    advancedbutton,
    pos=0.83
]
{10C} (Manual Memory from ON.south);

% 11 Simple UI Config from OFF -----------------------------------------
% ----------------------------------------------------------------------
{ [on background layer]
    \node (Simple UI Config from OFF)[
        simpleUIconfigmenu,
        right=\nodespaceh of Ramp Extras Config from ON.north east,
        anchor=north west
    ]
    {
        \spacingnoextra
        \begin{tblr-advancedconfigmenu}{}
            Simple UI Config &\\
            1 & Floor: nC = n of 150\\
            2 & Ceiling: nC = 151 \char"2212{} n\\
            % Unicode character hex code 2212: Minus Sign
            3 & Step count: 1\textendash{}150C\\
            4 & Simple UI's turbo style\\
              & {0C/1C/2C\\
                 \char"2192{} see Ramp Extras, 4}\\
                 % Unicode character hex code 2192: Rightwards Arrow
        \end{tblr-advancedconfigmenu}
    };
}

\path (OFF.north) edge[
    ->,
    advancedline,
    out=90,
    in=323,
    out looseness=1.1,
    in looseness=1
]
node[
    advancedbutton,
    pos=0.79
]
{10H} (Simple UI Config from OFF.278);

% Misc Config from OFF
{ [on background layer]
    \node (Misc Config from OFF)[
        advancedconfigmenu,
        right=\nodespaceh of Simple UI Config from OFF.north east,
        anchor=north west
    ]
    {
        \spacingnoextra
        \begin{tblr-advancedconfigmenu}{
            row{2}     = {belowsep=2pt},
            cell{2}{1} = {c=2}{}
        }
            Misc Config\inherited{} &\\
            Hardware-specific &\\
            1 & \tintrampboxadv{Tint Ramp} style\\
              & {0C = Smooth ramp\\
                 1C = Middle tint only\\
                 2C = Channel toggle\\
                 3+C = Stepped ramp\\
                 with 3+ steps}\\
            2 & Jump Start level\\
              & 1\textendash{}150C\\
        \end{tblr-advancedconfigmenu}
    };
}

\path (OFF.north) edge[
    ->,
    advancedline,
    out=90,
    in=304,
    out looseness=1,
    in looseness=0.8
]
node[
    advancedbutton,
    pos=0.84
]
{9H} (Misc Config from OFF.252);

% 12 Momentary Moon from Lockout ---------------------------------------
% ----------------------------------------------------------------------
\node (Momentary Moon from Lockout)[
    anyconfig,
    below right=1.2cm and 3.3cm of Lockout.center,
    anchor=north
]
{
    \firstline{Momentary Moon}
    Lowest floor (of smooth\\
    vs stepped ramp)
};

\path (Lockout.south) edge[
    ->,
    anyline momentary,
    out=270,
    in=180,
    looseness=1.1
]
node[
    anybutton,
    pos=0.75
]
{1H} (Momentary Moon from Lockout.west);

% Momentary Low from Lockout
\node (Momentary Low from Lockout)[
    anyconfig,
    below=\nodespacev of Momentary Moon from Lockout.south west,
    anchor=north west
]
{
    \firstline{Momentary Low}
    Highest floor, or manual\\
    memory level if set
};

\path (Lockout.south) edge[
    ->,
    anyline momentary,
    out=270,
    in=180,
    looseness=1
]
node[
    anybutton,
    pos=0.745
]
{2H} (Momentary Low from Lockout.west);

% Next Channel Mode from Lockout
\node (Next Channel Mode from Lockout)[
    anyconfig,
    below=\nodespacev of Momentary Low from Lockout.south west,
    anchor=north west
]
{\oneline{Next Channel Mode}};

\path (Lockout.south) edge[
    ->,
    anyline,
    out=270,
    in=180,
    looseness=1
]
node[
    anybutton,
    pos=0.765
]
{3H} (Next Channel Mode from Lockout.west);

% 13 Aux LED Color from OFF right Lockout ------------------------------
% ----------------------------------------------------------------------
{ [on background layer]
    \node (Aux LED Color from OFF right Lockout)[
        advancedconfig,
        above right=1.2cm and 3.3cm of Lockout.center,
        anchor=south
    ]
    {
        \firstline{Aux LED Color\inherited{}}
        Red \nextaux{} Yellow \nextaux{} Green \nextaux{}
        Cyan \nextaux{}\\
        Blue \nextaux{} Purple \nextaux{} White \nextaux{}\\
        Disco \nextaux{} Rainbow \nextaux{}
        \textboxvoltageadv{} \nextauxagain{}
    };
}

% Aux LED settings extra node in Off
\node (Aux LED settings extra node in Off)[
    below=0.15cm of Aux LED Color from OFF right Lockout.south east,
    anchor=north east,
    inner xsep=0.05cm,
    inner ysep=0cm,
    align=right
]
{\infotextadvanced{in Off}};

% Aux LED settings extra node in Lockout
\node (Aux LED settings extra node in Lockout)[
    below=0.15cm of Aux LED Color from OFF right Lockout.south west,
    anchor=north west,
    inner xsep=0.05cm,
    inner ysep=0cm,
    align=left
]
{\infotextadvanced{in Lockout}};

\path (OFF right.north) edge[
    ->,
    advancedline,
    out=90,
    in=0,
    looseness=1.1
]
node[
    advancedbutton,
    pos=0.65
]
{7H} (Aux LED Color from OFF right Lockout.east);

\path (Lockout.north) edge[
    ->,
    advancedline,
    out=90,
    in=180,
    looseness=1.1
]
node[
    advancedbutton,
    pos=0.65
]
{7H} (Aux LED Color from OFF right Lockout.west);

% Aux LED Pattern from OFF right Lockout
\node (Aux LED Pattern from OFF right Lockout)[
    advancedconfig,
    above=\nodespacev of
    Aux LED Color from OFF right Lockout.north west,
    anchor=south west
]
{
    \firstline{Aux LED Pattern\inherited{}}
    Off \nextaux{} Low \nextaux{} High \nextaux{}
    Blinking \nextaux{}\\
    2 Low Blinks \nextaux{} 1 Low Blink \nextauxagain{}
};

\path (OFF right.north) edge[
    ->,
    advancedline,
    out=90,
    in=0,
    looseness=0.9
]
node[
    advancedbutton,
    pos=0.7
]
{7C} (Aux LED Pattern from OFF right Lockout.east);

\path (Lockout.north) edge[
    ->,
    advancedline,
    out=90,
    in=180,
    looseness=0.9
]
node[
    advancedbutton,
    pos=0.7
]
{7C} (Aux LED Pattern from OFF right Lockout.west);

% Auto-lock Config from Lockout
\node (Auto-lock Config from Lockout)[
    advancedconfig,
    above left=2.325cm and 1.8cm of
    Aux LED Pattern from OFF right Lockout.north west,
    anchor=north east
]
{
    \firstline{Auto-lock\inherited{}}
    0C = Disable\\
    nC = n minutes
};

\path (Lockout.north) edge[
    ->,
    advancedline,
    out=90,
    in=303,
    looseness=0.8
]
node[
    advancedbutton,
    pos=0.85
]
{10H} (Auto-lock Config from Lockout.255);

% 14 Tactical Mode from OFF right --------------------------------------
% ----------------------------------------------------------------------

% Tactical Mode Config
{ [on background layer]
    \node (Tactical Mode Config)[
        advancedconfigmenu,
        right=1.2cm of Blinky Modes border south-east,
        anchor=south west
    ]
    {
        \spacingnoextra
        \begin{tblr-advancedconfigmenu}{
            column{1} = {0.45cm}
        }
            Tactical Mode Config &\\
            1\textendash{}3 & Tactical Slots 1\textendash{}3\\
                            & {1\textendash{}150C = Brightness\\
                               0C = Last-used strobe/\\
                               mood mode\\
                               151C = Party strobe\\
                               152C = Tactical strobe\\
                               153+C = Other strobes}\\
        \end{tblr-advancedconfigmenu}
    };
}

% Tactical Mode from OFF right
\node (Tactical Mode from OFF right)[
    advancedconfig,
    above=0.8cm of Tactical Mode Config.north,
    anchor=south
]
{
    \firstline{Tactical Mode}
    1H = Tactical Slot 1 (high)\\
    2H = Tactical Slot 2 (low)\\
    3H = Tactical Slot 3 (strobe)
};

\path (Tactical Mode from OFF right.south) edge[
    ->,
    advancedline,
    out=270,
    in=90,
    looseness=1
]
node[
    advancedbutton,
    pos=0.4
]
{7H} (Tactical Mode Config.north);

\path (OFF right.south) edge[
    -{<[sep=1.9cm]>},
    advancedline,
    out=270,
    in=40,
    looseness=1
]
node[
    advancedbutton,
    pos=0.84
]
{6C} (Tactical Mode from OFF right.45);

% Tactical Mode extra node aux LED settings
\node (Tactical Mode extra node aux LED settings)[
    above=0.05cm of Tactical Mode from OFF right.north west,
    anchor=south west,
    inner xsep=0.05cm,
    inner ysep=0cm,
    align=left
]
{
    \infotextadvanced{Lockout mode's}\\
    \infotextadvanced{aux LED settings}
};

% 15 Factory Reset from OFF right --------------------------------------
% ----------------------------------------------------------------------
\node (Factory Reset from OFF right)
at (current page text area.south east)[
    anyconfig factoryreset,
    xshift=-0.45cm,
    anchor=south east
]
{
    \firstline{Factory Reset}
    Or reconnect power\\
    while holding for 4s
};

\path (OFF right.south) edge[
    ->,
    anyline,
    out=270,
    in=170,
    out looseness=0.7,
    in looseness=0.45
]
node[
    anybutton,
    pos=0.79,
    label={ [align=left] right:
        \infotextany{Hold for 4s}
    }
]
{13H} (Factory Reset from OFF right.west);

% Version Check from OFF right
\node (Version Check from OFF right)[
    anyconfig,
    above=1.5cm of Factory Reset from OFF right.north east,
    anchor=south east
]
{
    \firstline{Version Check}
    MODEL-YYYY-MM-DD\\
    (-SINCE-DIRTY)
};

\path (OFF right.south) edge[
    ->,
    anyline,
    out=270,
    in=170,
    out looseness=0.7,
    in looseness=0.45
]
node[
    anybutton,
    pos=0.77
]
{15+C} (Version Check from OFF right.west);

% 16 Advanced UI switch from OFF right ---------------------------------
% ----------------------------------------------------------------------
\node (Advanced UI switch from OFF right)
at (current page text area.east)[
    advancedUI,
    xshift=-0.45cm,
    yshift=-2.4cm,
    anchor=north east
]
{Advanced UI\vphantom{\S}};

{ [on above layer]
    \path (OFF right.east) edge[
        ->,
        advancedUI switch line,
        shorten <=-0.4pt, % out
        out=0,
        in=90,
        looseness=1.2
    ]
    node[
        simplebutton,
        pos=0.63
    ]
    {10H} (Advanced UI switch from OFF right.north);
}

% Simple UI switch from OFF right
\node (Simple UI switch from OFF right)
at (current page text area.east)[
    simpleUI,
    xshift=-0.45cm,
    yshift=2.4cm,
    anchor=south east
]
{Simple UI};

\path (OFF right.east) edge[
    ->,
    simpleUI switch line,
    shorten <=-0.4pt, % out
    out=0,
    in=270,
    looseness=1.2
]
node[
    advancedbutton,
    pos=0.65
]
{10C} (Simple UI switch from OFF right.south);

% Line from Factory Reset to Simple UI

% -0.3pt: 0.5 line thickness for vertical alignment for semithick
\coordinate (Factory Reset right) at
    ($(Factory Reset from OFF right.east) + (0.45cm-0.3pt, 0cm)$);
\coordinate (Simple UI switch right) at
    ($(Simple UI switch from OFF right.east) + (0.45cm-0.3pt, 0cm)$);

{ [on background layer]
    \path[
        -{
            >[
                scale length=1.15, % for semithick
                scale width=1.15 % for semithick
            ]
        },
        anyline factoryreset simpleUI
    ]
    (Factory Reset from OFF right.east)
    -- (Factory Reset right)
    -- (Simple UI switch right)
    -- (Simple UI switch from OFF right.east);
}

% Simple UI switch extra node Extended Simple UI
\node (Simple UI switch extra node Extended Simple UI)[
    extendedsimpleUI,
    above=0.2cm of Simple UI switch from OFF right.north east,
    anchor=south east
]
{\normalsize Extended Simple UI};

% Extended Simple UI description
\node (Extended Simple UI description)[
    above=0.05cm of
    Simple UI switch extra node Extended Simple UI.north east,
    anchor=south east,
    inner xsep=0.05cm,
    inner ysep=0cm,
    align=right
]
{
    \infotextsimple{On some lights; typically enables}\\
    \infotextsimple{toggle ramp style, sunset timer,}\\
    \infotextsimple{and aux LED pattern and color.}
};

% 17 Anduril 2 UI ------------------------------------------------------
% ----------------------------------------------------------------------
{ [on background layer]

\node (Anduril 2 UI)
at (current page text area.north east)[
    andurilinfo,
    anchor=north east
]
{
    \Huge\textbf{Andúril 2 UI\hspace*{0.15pt}}\\[8pt]
    % +0.15pt for more width than Aux LED Color for Voltage
    \normalsize Release 2026-08-12\\[4pt]
    Diagram Release 03
    \spacingv
    https://github.com/dirtydancing/\\
    anduril-tikz-diagram
};

% Aux LED Color for Voltage
\node (Aux LED Color for Voltage)[
    voltagecolorinfo,
    below=\nodespacev of Anduril 2 UI.south east,
    anchor=north east
]
{
    \oneline{Aux LED Color for Voltage}
    \spacingv
    \textboxauxledcfv{}\\[3pt]
    \smoothPOVDbox{Smooth POVD} (\char"007E{}60 steps)
    % Unicode character hex code 007E: Tilde
    \dimmableRGB{}\\
    \char"2192{} for POVD, see Voltage Config, 2
    % Unicode character hex code 2192: Rightwards Arrow
    \spacingv
    Single-color aux: fast-blink 3.3\textendash{}2.9V
};

% Button Presses
\node (Button Presses)[
    buttonpressesinfo,
    left=\nodespaceh of Anduril 2 UI.north west,
    anchor=north east
]
{
    \spacingnoextra
    \begin{tblr-buttonpresses}{}
        Button Presses\vphantom{Config} & &\\
        \tikzmarknode{buttonpresses-1C}
            {\textbasicuse{1C}} &
            \textbasicuse{:} &
        \tikzmarknode{buttonpresses-Click}
            {\textbasicuse{Click\hspace{0.015cm}}}\\
        \tikzmarknode{buttonpresses-1H}
            {\textbasicuse{1H}} &
            \textbasicuse{:} &
        \tikzmarknode{buttonpresses-Hold}
            {\textbasicuse{Hold}}\\
        2C & : & Click, Click\\
        2H & : & Click, Hold\\
        3C & : & Click, Click, Click\\
        3H & : & Click, Click, Hold\\
        {\infotextadvancedconfigmenu{In a config menu, release}\\
         \infotextadvancedconfigmenu{button after the desired}\\
         \infotextadvancedconfigmenu{number of blinks; when}\\
         \infotextadvancedconfigmenu{entering a value, 1H = 10C}} & &\\
    \end{tblr-buttonpresses}
};

\node (Button Presses basic use fit)[
    button presses basic use fit,
    fit=(buttonpresses-1C)
    (buttonpresses-Click)
    (buttonpresses-1H)
    (buttonpresses-Hold)
]
{};

% Defaults vary by light
\node (Defaults vary by light)[
    defaultsvaryinfo,
    below=0.1cm of Button Presses.south west,
    anchor=north west
]
{Defaults vary by light.};

% Legend
\matrix (Legend)[
    matrixlegendinfo,
    left=\nodespaceh of Button Presses.north west,
    matrix anchor=north east
]
{
    & \textbasicuse{All the user needs to know
      for basic use\vphantom{y}}\\
    & Simple UI and Advanced UI function\\
    & Simple UI function only\\
    & \infotextadvanced{Advanced UI function only}\\
    & Continuous effect\vphantom{y}\\
    & Momentary effect\\
    \normalsize{\dimmableRGB{}}&
    On lights with dimmable RGB aux LEDs\\
    \normalsize\textbf{\inherited{}}&
    \textinherited{Inherited by Simple UI}\\
};

\path (Legend-1-1.west) edge[
    -,
    anyline basic use legend,
    shorten <=0.25pt, % out (0.4pt overhang)
    shorten >=0.25pt, % in (0.4pt overhang)
    out=0,
    in=180
]
node[
    anybutton basic use legend,
    pos=0.5
]
{\footnotesize 1C} (Legend-1-1.east);

\path (Legend-2-1.west) edge[
    -,
    anyline legend,
    out=0,
    in=180
]
node[
    anybutton legend,
    pos=0.5
]
{\footnotesize 1C} (Legend-2-1.east);

\path (Legend-3-1.west) edge[
    -,
    simpleline legend,
    out=0,
    in=180
]
node[
    simplebutton legend,
    pos=0.5
]
{\footnotesize 1C} (Legend-3-1.east);

\path (Legend-4-1.west) edge[
    -,
    advancedline legend,
    out=0,
    in=180
]
node[
    advancedbutton legend,
    pos=0.5
]
{\footnotesize 1C} (Legend-4-1.east);

\path (Legend-5-1.west) edge[
    -,
    anyline legend,
    out=0,
    in=180
]
(Legend-5-1.east);

\path (Legend-6-1.west) edge[
    -,
    anyline momentary legend,
    out=0,
    in=180
]
(Legend-6-1.east);

% Anduril User Manual
\node (Anduril User Manual)
at (current page text area.north)[
    usermanualinfo,
    anchor=north
]
{
    \firstline{Recommended Reading}
    ToyKeeper's Andúril User Manual:\\
    https://toykeeper.net/anduril/manual
};

} % for ending on background layer

\end{tikzpicture}

\end{document}
